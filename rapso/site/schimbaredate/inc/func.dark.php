...
function login()
{
	if(isset($_POST['submit']))
	{
		...
		if($row[0] > 0)
		{
			...
			echo "<div class=\"msg_succes\" align=\"center\" style=\"width:150px;\">Logare reusita.</div>";
...
function loadcontent()
{
	if(isset($_GET['page']))
	{
		...
		{
			$error=1;
			echo error("Modul inexistent sau indisponibil momentan.Va rugam reveniti!");
...
function nume_regat($empire)
{
	if($empire == '1')
	{
		echo  "<font color='red'>Regat Rosu</font>";
	}
	if($empire == '2')
	{
		echo "<font color='yellow'>Regat Galben</font>";
	}
	if($empire == '3')
	{
		echo "<font color='#00ffff'>Regat Albastru</font>";
	}	
}
...
function recuperare_pw()
{
		...
		if(isset($_POST['recuperare']))
		{
			...
			Salut ".$username. "\r\n" ."
			Ai cerut resetarea parolei dar inainte de asta trebuie sa confirmi emailul.Click pe linkul de mai jos : ". "\r\n" ."
			http://metin2rapsodia.ro/schimbaredate/index.php?page=recuperare-pw&cont=".$username."&cod=".$rec."
							";
							$headers = 'Acest mesaj a fost generat automat.Va rugam nu raspundeti.';
		
							new mail($to, $subject, $message, $headers);
							echo succes("Un email de confirmare a fost trimis.Urmeaza instructiunile primite.");
					} else { echo error("Contul sau email nu se potrivesc.");} 
				} else { echo error("SPAM!Introduceti corect codul din imagine."); }
			}
		}
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function schimbare_pw()
{
	...
	if(isset($_POST['passwordchangerequest']))
	{
		...
		echo succes("Un email a fost trimis la ".$tr->email."");	
		$email = $tr->email;
		$to      = $email;
		$subject = 'Confirmare schimbare parola!';
		$message = "Pentru a confirma ca sunteti de acord cu schimbarea parolei accesati linkul de mai jos ". "\r\n" ."http://metin2rapsodia.ro/schimbaredate/index.php?page=schimbare-pw&cod=".$cod."";
...
function schimbare_pw_confirmata()
{
	...
	if(isset($_POST['SubmitLostPasswordCodeForm']))
	{
		...
		if($newpw !=NULL)
		{
			...
			echo succes("Parola a fost modificata cu succes.");
		} else { echo error("Introdu noua parola!"); }
	} else { }
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function parola_depozit()
{
	...
	if(isset($_POST['sendStoragePassword']))
	{
		...
		$to      = $email;
		$subject = 'Parola depozit!';
		$message = "Ati cerut parola dumneavoastra de la depozit.Aceasta este : ".$password."";
		new mail($to, $subject, $message);
		$data = date("h:i:s d/m/Y");
			mysql_query("Insert into web.dev_player_log (account,data,actiune) values ('$cont','$data','Recuperare parola depozit.')");
		echo succes("Parola a fost trimisa la ".$email.".");
	}
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function cod_secret()
{
	...
	{
	include("configurare.php");
		echo succes("Vei primi in cel mai scurt timp un email continand codul secret.");
		$log = $_SESSION['user'];
		...
		$subject = 'Cod secret!';
		$message = "Salut ".$log."!". "\r\n" ."
		Pentru o siguranta mai mare, pastreaza acest cod secret format intre 6-14 caractere , litere/cifre (inclusiv 6 sau 14) si iti va folosi.  .". "\r\n" ."
		Codul tau este : ".$cod."". "\r\n" ."

		Memoreaza-l sau noteaza-l!". "\r\n" ."
		...
		new mail($to, $subject, $message);
		$data = date("h:i:s d/m/Y");
			mysql_query("Insert into web.dev_player_log (account,data,actiune) values ('$cont','$data','Recuperare cod secret.')");
	}
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function usernamerp()
{
	...
	{
	include("configurare.php");
		echo succes("Vei primi in cel mai scurt timp un email continand username-ul tau.");
		$xx = mysql_fetch_object(mysql_query("Select * from account.account")) or die(mysql_error());
		$usernamerp = $xx->login;
		$email = $xx->email;
		$to      = $email;
		$subject = 'Username-ul tau!';
		$message = "Salut straine!". "\r\n" ."
		Vezi sa nu-ti mai uiti Username-ul de acum!  .". "\r\n" ."
		Username-ul tau : ".$usernamerp."". "\r\n" ."
		...
		new mail($to, $subject, $message);
		$data = date("h:i:s d/m/Y");
			mysql_query("Insert into web.dev_player_log (account,data,actiune) values ('$cont','$data','Recuperare username.')");
	}
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
function cod_securitate()
{
	...
	{
	include("configurare.php");
		echo succes("Vei primi in cel mai scurt timp un email continand codul de stergere.");
		$log = $_SESSION['user'];
		$xx = mysql_fetch_object(mysql_query("Select * from account.account where login='$log'")) or die(mysql_error());
		$cod = $xx->social_id;
		$email = $xx->email;
		$to      = $email;
		$subject = 'Cod siguranta [stergere caractere]!';
		$message = "Salut ".$log."!". "\r\n" ."
		Pentru o siguranta mai mare, Inainte de stergerea caracterului, este nevoie de introducerea unui cod special.". "\r\n" ."
		Codul tau este : ".$cod."". "\r\n" ."
		...
		new mail($to, $subject, $message);
		$data = date("h:i:s d/m/Y");
			mysql_query("Insert into web.dev_player_log (account,data,actiune) values ('$cont','$data','Recuperare cod securitate.')");
	}
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
function sterge_cont()
{
	...
	{
			echo succes("Vei primi in cel mai scurt timp un email.Urmeaza instructiunile cu atentie.");
			$log = $_SESSION['user'];
			...
			$to      = $email;
			include("configurare.php");
			$subject = 'Confirmare stergere account!';
			$message = "Salut ".$log."!". "\r\n" ."
			Ati solicitat stergerea contului dumneavoastra.Va rugam sa confirmati asta apasand pe linkul de mai jos.". "\r\n" ."
			http://metin2rapsodia.ro/schimbaredate/index.php?page=stergere-cont&cont=".$log."&cod=".$cod."". "\r\n" ."
			Puteti anula stergerea contului in decurs de 7 zile din panou dumneavoastra de control!". "\r\n" ."
	...
	if(isset($_POST['adauga']))
	{
		if(isset($_GET['cont']))
		{
			...
			else
			{
				...
				echo '<meta http-equiv="refresh" content="4;url=index.php?page=edit_cont&cont='.$cont.'">';
			}
		}
	}
}
function plus_jmonezi()
{
	if(isset($_POST['adauga']))
	{
		if(isset($_GET['cont']))
		{
			...
			{
				$ncoins = $old->jcoins;
				$jcoins = $cantitate + $ncoins;
				$data = date("H:i:s d-M-y ");
				mysql_query("Update account.account set jcoins='$jcoins' where id='$cont'");
				echo succes("".$cantitate." jetoane au fost adaugate.Acum are in total ".$jcoins." jetoane dragon");
				mysql_query("Insert into web.dev_log_monezi (data,admin,actiune) values ('$data','".$_SESSION['user']."','A adaugat ".$cantitate." jetoane userului ".$old->login."')") or die(mysql_error()); // LOG
				echo '<meta http-equiv="refresh" content="4;url=index.php?page=edit_cont&cont='.$cont.'">';
			}
		}
	}

...

{
	...
	$lp = mysql_query("Select pret from ".$web_db.".dev_is_items where id='$id'");
...

	{
		...
		mysql_query("Update ".$web_db.".dev_is_items set pret='$pret',last_price='$last_price' where id='$id'") or die(mysql_error());	
...
$i = 0;
  ...
  $query = mysql_query("Select * from ".$web_db.".dev_is_items  ORDER BY id DESC limit ".($page_rows * ($pagenum - 1)).", ".$page_rows."");
...
$cat = mysql_fetch_object(mysql_query("Select * from ".$web_db.".dev_is_cat where id='$item->cat_id'"));
...
<tr> 
   ...
   <td width='245'>  <font color='white'>$item->descriere</font></td>
    ...
    <td class="tabel5"><?=$item->pret?> monezi </td>

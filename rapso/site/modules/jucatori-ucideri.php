...
$result2 = mysql_query("Select * from player.player WHERE name NOT LIKE '[%]%' ORDER BY player.red_kills+player.yellow_kills+player.blue_kills DESC limit ".($page_rows * ($pagenum - 1)).", ".$page_rows.""); 
...
while($acs = mysql_fetch_object($result2))
{
 ...
 $suma = mysql_fetch_array(mysql_query("SELECT SUM(yellow_kills + blue_kills + red_kills ) AS Tot FROM player.player WHERE name='".$acs->name."'"));
	?>

<tr>
                        <td width="60" rowspan="3" align="center" class="collect"><img src="images/clase/<?=$acs->job?>.png" width="27" height="50" /></td>
                        </tr>
						<td align="center" class="collect"><?=$acs->yellow_kills?> ucideri la Galben<p> <?=$acs->blue_kills?> ucideri la Rosu<p> <?=$acs->red_kills?> ucideri la Albastru</td> <p><td align="center" class="collect">Total: <?php  echo $suma["Tot"]; ?> </td></p>
                        <tr>
                        <td width="232" align="left" class="iR_stats"><center><?=$nr?>.<a href="index.php?page=jucator&id=<?=$acs->id?>"><?=$acs->name?></a></center></td>
						<td align="center" class="collect"><?=nume_regat($empire)?> <a href="index.php?page=bresla&id=<?=$gid?>"><?=$bresla?></a></td>
                        <tr>

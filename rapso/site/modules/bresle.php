...
if ((!isset($getID)) || (!is_numeric($getID)) || ($getID < 1)) { $pagenum = 1; }
	...
	$result = mysql_query("SELECT * FROM guild WHERE guild.name NOT LIKE 'STAFF' ORDER BY win desc, level desc, exp desc, exp desc, name asc limit 0,100");
...
$result2 = mysql_query("SELECT * FROM guild WHERE guild.name NOT LIKE 'STAFF' ORDER BY win desc, level desc, exp desc, exp desc, name asc limit ".($page_rows * ($pagenum - 1)).", ".$page_rows.""); 
...
<tr class="top">
	<td><?=$nr?></td>
	<td><a href="index.php?page=bresla&id=<?=$sGuild->id?>"><?=$sGuild->name?></a></td>

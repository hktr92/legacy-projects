...
<?PHP
  ...
  echo'<h2>Administrare - ad&#259;ugare jetoane(multi utilizator)</h2>';
  echo'<p>&#206;n aceast&#259; zon&#259; pute&#355;i ad&#259;uga Jetoane la mai mul&#355;i utilizatori. Aceast&#259; func&#355;ie necesit&#259; un anumit format:<br/>
  <b>ID-ul Contului||Jetoanele</b> Exemplu: (ExCont||1000)</p>';
    ...
    for($x=0;$x<$mengeZeilen;$x++) {
      ...
      if(checkAnum($userLogin) && checkInt($userCoins)) {
        ...
        if(mysql_num_rows($qryUser)>0) {
          $sqlCoins = "UPDATE account.account SET jcoins=jcoins+".$userCoins." WHERE login='".$userLogin."' LIMIT 1";
          $qryCoins = mysql_query($sqlCoins,$sqlServ);
          
          if($qryCoins) 
          {
            echo'<p><b>Linia '.($x+1).' <img src="./img/icons/success.gif"/>: Jetoanele contului <u>'.$userLogin.'</u> au fost setate la <u>'.$userCoins.'</u> !</b></p>';
          }
          else 
          {
            $fehlerZeilen[]=$x;
...
?>
<form method="POST" action="index.php?s=admin&a=add_jcoins_multi">
<table>
  <tr>
    <th class="topLine">
      Introduce&#355;i datele:<br/><br/>
      ID-ul contului||jetoanele<br/>
      Exemplu: (ExCont||1000)
    </th>
    <td class="thell">
